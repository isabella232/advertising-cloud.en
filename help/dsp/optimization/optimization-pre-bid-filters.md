...
| [!UICONTROL Viewability IAS] | Sets a required minimum viewability percentage, using historical data from [!DNL IAS]. You'll bid on an impression when the specified threshold is met. | This filter works best as more data is pulled in through a campaign-level [!DNL IAS] integration.<br><br>When the campaign is configured to use [!DNL IAS] data, the best practice is to use this filter with the package optimization goal "[!UICONTROL Lowest vCPM (IAS)]." If the integration isn't enabled, use this filter with the optimization goal "[!UICONTROL Lowest CPM]." |
